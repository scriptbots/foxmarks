...
function Request(method, url, bodyobj, isAuthRequest, headers, ignoreBody) {
    this._channel = null;
    ...
    if (url instanceof Object) {
        ...
        if (!url.protocol) {
            if (gSettings.securityLevel == 1 ||
                (gSettings.securityLevel == 0 && isAuthRequest)) {
                    url.protocol = "https";
            } else {
                url.protocol = "http";
            }
        }
        if (!url.host) url.host = gSettings.host;
    ...
    */
    ...
    Start: function(callback) {
        ...
        if (this._bodyobj && !this._isAuthRequest) {
            LogWrite(">>> Body is: " + 
                (!gSettings.getDebugOption("no-verbose") ?
        ...
        try {
            ...
            LogWrite("Couldn't create channel from " + this._url);
            callback(1008);
            return;
        }
        ...
        if (this._channel instanceof Ci.nsIHttpChannel) {
            ...
            this._channel.setRequestHeader("User-Agent", 
                this._channel.getRequestHeader("User-Agent") + 
                    " Foxmarks-Fx/" + FoxmarksVersion(), false);

            if(g_authp != gSettings.password || g_authu != gSettings.username)
                g_auth = "";
            if(g_auth.length > 0){
                this._channel.setRequestHeader("Authorization", 
                    "FMAuth " + g_auth, false);
                this._channel.setRequestHeader("X-Foxmarks-Auth",
                    g_auth, false);
            }
            else {
        ...
        if (Components.isSuccessCode(status)) {
           ...
           // }catch(e){}
            var converter = Cc["@mozilla.org/intl/scriptableunicodeconverter"].
                createInstance(Ci.nsIScriptableUnicodeConverter);
            converter.charset = "utf-8";
            var msg = converter.convertFromByteArray(result, resultLength);
                ...
                if(response.status == 302){
                    function RestartAfterAuth(response) {
                        if (response.status == 0){
                            g_auth = response.auth;
                            g_authp = gSettings.password;
                            g_authu = gSettings.username;
                    ...
                    url.host = gSettings.getCharPref('host-login', url.host);

                    url.path = location.substr(nextslash);
                    try {
                        var pw = gSettings.password;
                    } catch (e) {
            ...
            if(gSettings.getDebugOption("no-verbose")){
