...
function loadDatasourceSet(allItems) {
    ...
    for(type in SYNCSET){
        if(obj[type] === undefined){
            if(gSettings.isSyncEnabled(type) || allItems){
...
SyncServer.prototype = {
    ...
    _createSyncEngine: function(datasource){
        var cls = gSettings.useOwnServer ?
            OwnSyncEngine : 
            Syncd2SyncEngine;
            ...
            var mycallback = function(statuscode){
                if(statuscode == 0 || statuscode == 444){
                    ...
                    if(statuscode == 444){
                        ...
                        gSettings.setSyncEnabled(ds.syncType, false);
                        statuscode = 0;
                    }
                    if(ds && ds.syncType !== undefined){
                        gSettings.SyncComplete(ds.syncType);
                        SetProgressComponentStatus(ds.syncType, "end");
                    }
                    ds = list.shift();
                    if(ds){
                        that._syncEngine = that._createSyncEngine(ds);
                        SetProgressComponentStatus(ds.syncType, "start");
                        if(arg2 !== undefined){
    ...
    sync: function(prevState, callback){
        SetProgressMessage("progress.syncing");
...
SyncEngine.prototype = {
    ...
    sync: function(prevState, callback) {
        ...
        var funcSyncComplete = function(finalLcs,
                ...
                conflicts,
                showedUI) {
            //SetProgressMessage("progress.writing");
        ...
        var funcWroteServerChanges = function(obj) {
            ...
            if (scs.set.length > 0) {
                //SetProgressMessage("progress.loading");
        ...
        if (!gSettings.getHaveSynced(self._datasource.syncType)) {
            self.status(funcCheckStatus);
        }
        else {
            if(gSettings.mustUpload(self._datasource.syncType)){
                LogWrite("Forced Upload: Pin Reset");
                self.upload(function(response){
                    if(self._responseOK(response, callback)){
                        gSettings.setMustUpload(self._datasource.syncType, false);
                        callback(response);
                    }
                });
                return;
            ...
            //SetProgressMessage("progress.downloading");
...
Syncd2SyncEngine.prototype._args = function(dict, ignoreUpload) {
    ...
    if (gSettings.machineId) {
        if (!dict["log"]) {
            dict["log"] = {};
        }
        dict["log"]["mid"] = gSettings.machineId;
    }
    if (gSettings.viewId) {
        if(!ignoreUpload){
            dict.view = gSettings.viewId;
        }
    }
    return dict;
}
...
Syncd2SyncEngine.prototype.upload = function(callback) {
            ...
            self.request = new Request("POST", 
                {
                    ...
                    host: gSettings.getServerHost(self._datasource.syncType)
                }, 
                self._args({ "commands" : cs },
                    gSettings.mustUpload(self._datasource.syncType)
                )
            );
    ...
    if (gSettings.viewId && !gSettings.mustUpload(self._datasource.syncType)) {
...
Syncd2SyncEngine.prototype._uploadWithProfile = function(callback) {
    ...
    var funcCompared = function(status, cs) {
        if (self._responseOK(status, callback)) {
            if (cs.length) {
                //SetProgressMessage("progress.uploading");
                self.request = new Request("POST",
                    {
...
OwnSyncEngine.prototype.testDupURL = function(){
    ...
    // and bookmark
    if(gSettings.isSyncEnabled("bookmarks") && 
        gSettings.isSyncEnabled("passwords")){
        if(gSettings.getUrlWithUsernameAndPassword("passwords") ==
            gSettings.getUrlWithUsernameAndPassword("bookmarks")){
                return true;
        }
    }
    return false;
...
OwnSyncEngine.prototype._getServerChanges = function(callback) {
    ...
    var funcProcessFile = function(response) {
        if (response.status == 304) {   // Not modified.
            callback(0, { continuous: true, mscs: new Commandset(), 
                token: gSettings.getToken(self._datasource.syncType), etag: response.etag });
            return;
        }
        if (self._responseOK(response, callback)) {
            if (response.token != gSettings.getToken(self._datasource.syncType)) {
    ...
    var serveretag = gSettings.getEtag(this._datasource.syncType);
    if (serveretag.length) {
        headers["If-None-Match"] = serveretag;
    }
    self.request = new Request("GET", 
        gSettings.getUrlWithUsernameAndPassword(self._datasource.syncType),
        null, false, headers);
...
OwnSyncEngine.prototype._writeServerChanges = function(lcs, lns, sco, conflicts,
    ...
    var funcGotCommandset = function(status, cs) {
        var header = {};
        if (!gSettings.disableIfMatchOnPut && sco.etag && sco.etag.length) {
...
SyncEngine.prototype.merge = function(local, callback) {
    ...
    var funcFetched = function() {
        //SetProgressMessage("progress.merging");
        if (local) {
...
Syncd2SyncEngine.prototype._fetchBaseline = function(callback) {
    ...
    var funcGotIt = function(status) {
        if (self._responseOK(status, callback)) {
            self._baseline.hash = gSettings.hash;
            callback(0);
        }
    };

    if (self._baseline && self._baseline.hash == gSettings.hash) {
    ...
    try {
        ...
        var request = new Request("POST",
            ...
            self._args({ rev: gSettings.GetSyncRevision(self._datasource.syncType), depth: "all", 
                    log: { "error": e.name }}));
...
Syncd2SyncEngine.prototype._completeTransaction = function(ns, sco, callback) {
    ...
    var funcFinishedWrite = function(status) {
        if (self._responseOK(status, callback)) {
            // gSettings.currentRevision = sco.toprev;
            gSettings.SetSyncRevision(self._datasource.syncType, sco.toprev);
            // gSettings.SyncComplete();
            callback(0);
        }
    };
...
Syncd2SyncEngine.prototype.getProfileNames = function(callback) {
    ...
    //SetProgressMessage("progress.gettingprofilenames");
    var request = new Request("POST",
        { path: "/user/profiles/getnames",
          host: gSettings.acctMgrHost },
        this._args({}) );
...
OwnSyncEngine.prototype.verifypin = function(pin, callback) {
    ...
    self.request = new Request("GET", gSettings.getUrlWithUsernameAndPassword(self._datasource.syncType));
    self.request.Start(funcDone);
}
...
OwnSyncEngine.prototype._completeTransaction = 
    ...
    var funcFinishedWrite = function(status) {
        if (self._responseOK(status, callback)) {
            gSettings.setToken(self._datasource.syncType, sco.token);
            if (sco.etag) gSettings.setEtag(self._datasource.syncType, sco.etag);
            gSettings.SyncComplete(self._datasource.syncType);
            callback(0);
        }
    };
    ns.Declone(funcFinishUp);
