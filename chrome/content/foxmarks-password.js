...
PasswordDatasource.prototype = {
    ...
    getBaselineName: function(){
        return "foxmarks-password-baseline-" + gSettings.hash + "json";
    },
    ...
    encrypt: function(str){
        var result = this.encryptor.encrypt(gSettings.pin, str);
        if(gSettings.forceGC) {
            Components.utils.forceGC();
        }
        return result;
    },
    decrypt: function(str){
        var result =  this.encryptor.decrypt(gSettings.pin, str);
        ...
        while(!goodPIN){
            ...
            gSettings.pin = newpin;
            result =  this.encryptor.decrypt(gSettings.pin, str);
            goodPIN = result.indexOf('password') >=  0;
            if(!goodPIN){
    ...
    _buildList: function(doEncrypt, includeCt, funcDone){
        ...
        NiceProcess(
            ...
            function(iter){
                ...
                if(login.httpRealm != SYNC_REALM &&
                        login.httpRealm != SYNC_REALM_PIN){
    ...
    ProvideNodes: function(Caller, AddNode, Complete) {
        var self = this;
        this._buildList(true, false, function(a){
            try {
                var now = new Date();
                var random_stuff = {
                    password: hex_md5(now.getTime().toString())
                };
                ...
                Complete.apply(Caller, [2]);
           }
        });
    },
    AcceptNodes: function(ns, callback) {
        var self = this;
        this._buildList(false, false, function(list){
            var nid, node;
...
PasswordSyncWatcher.prototype = {
   ...
   _getNotifyBox : function (win) {
    ...
    checkForChanges: function(){
        var self = this;
        if(!gSettings.isSyncEnabled("passwords") || !gSettings.haveSynced || self._suspended)
            return;
        ...
        this.server.getBaseline("passwords", function(status, ns){
            ...
            var funcCompareList = function(data){
