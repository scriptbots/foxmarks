            ...
            if(result.status == 3){
                ...
                if(label){
                    label.setAttribute("value",  result.phase == "end" ?
                        Bundle().GetStringFromName("progress.sync.done") :
                        Bundle().GetStringFromName("progress.sync.working")
                    );
                }
...
function onProgressLoad() {
    ...
    gFoxmarksService = Cc["@foxcloud.com/extensions/foxmarks;1"].
    ...
    for(x = 0; x < len; x++){
        ...
        label.setAttribute("value", gSettings.isSyncEnabled(type) ?
            Bundle().GetStringFromName("progress.sync.enabled") :
            Bundle().GetStringFromName("progress.sync.disabled")
        );
    }
    ...
    if (!okay) {
        ...
        newObserver.observe(null, null,
            { msg: Bundle().GetStringFromName("msg.busy"), status: 4 }.
            toSource());
    }
}
...
function onProgressCancel() {
    ...
    if (gCancelled) {
        ...
        window.arguments[1].msg = Bundle().GetStringFromName("msg.cancelled");
        return true;
    } else {
        gCancelled = true;
...
function onProgressHelp() {
    ...
    if (element) {
        ...
        window.arguments[1].helpurl =
            Bundle().formatStringFromName("url.error", [errmsg], 1);
